...
local function _combiner_cmd()
    ...
    vim.list_extend(cmd, {
        ...
        "--port",
        tostring(_config.combiner.port),
        "--host",
        _config.combiner.host or "127.0.0.1",
    })
...
function M._start_with_sharedserver()
    ...
    if not ok or result == false then
        log.error(
            "sharedserver.start() failed (%s) — falling back to direct start",
...
function M._create_client()
    ...
    client:connect(function(ok, err)
        if ok then
            ...
            log.info(
                "MCP client connected (%d tools, %d resources, %d prompts)",
                #client.tools,
                #client.resources,
                #client.prompts
            )
