...
function M.open()
    ...
    map("S", function()
        ...
        if not handle or not handle._mcp_token then
            vim.notify("[mcp-companion] Source buffer no longer has an MCP session", vim.log.levels.WARN)
            return
        end

        local sc_ok, sc = pcall(require, "mcp_companion.cc.session_commands")
        if not sc_ok then
            vim.notify("[mcp-companion] Session commands not available", vim.log.levels.WARN)
            return
        end

        sc.toggle_server_for_session(handle, srv_name, function(err, info)
            if err then
                vim.notify("[mcp-companion] Session toggle failed: " .. err, vim.log.levels.ERROR)
                return
            end
            vim.notify(
