...
function M.socket_path()
    if not _socket then
        ...
        local base = runtime and (runtime .. "/mcp-companion") or (vim.fn.stdpath("cache") .. "/mcp-companion-sock")
        vim.fn.mkdir(base, "p")
...
function M.start()
    ...
    if _reg_state == "done" or _reg_state == "pending" then
        return
    end
        ...
        callback = function(r)
            if r.status == 200 then
                ...
                if _reg_attempts < _MAX_REG_ATTEMPTS then
                    log.debug("channel: registration attempt %d failed (status %s); retrying", _reg_attempts, r.status)
                    vim.defer_fn(function()
                        M.start()
                    end, _REG_BACKOFF_MS)
                else
                    log.warn(
                        "channel: registration gave up after %d attempts (status %s): %s",
                        _reg_attempts,
                        r.status,
                        r.body or ""
                    )
...
function M.bind(token)
    if not token or not M.enabled() then
        return
    end
...
function M.deregister()
    if _reg_state == "idle" and not _socket then
        return
    end
    if _socket then
