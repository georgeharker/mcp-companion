// TypeScript-shape rendering of JSON Schemas for tool `describe` output.
//
// Adapted (near-verbatim) from pi-mcp-adapter's ts-shape.ts — MIT, © 2026 Nico Bailon
// (https://github.com/nicobailon/pi-mcp-adapter). Self-contained by design: no
// dependencies on the rest of that package. Conformance cases: test/schema-signature.test.ts.
...
export function renderSchemaSignature(inputSchema: unknown): string | null {
    try {
        ...
        const render = (schema: unknown): string | null => {
            ...
            if (schema.type === "object" || schema.properties !== undefined) {
                ...
                for (const [name, property] of Object.entries(schema.properties)) {
                    ...
                    // Field descriptions are the model's primary guidance at
                    // describe-time (it never sees the raw schema on the proxy
                    // path) — thread them as comments; defaults inline too.
                    const prop = isSchema(property) ? property : {}
                    const notes: string[] = []
                    ...
                    if (typeof prop.description === "string" && prop.description) {
                        const oneLine = prop.description.replace(/\s+/g, " ").trim()
                        notes.push(oneLine.length > 90 ? `${oneLine.slice(0, 87)}…` : oneLine)
                    }
                    const comment = notes.length > 0 ? ` // ${notes.join(" — ")}` : ""
                    properties.push(
                        `${formatPropertyName(name)}${required.has(name) ? "" : "?"}: ${rendered};${comment}`,
                    )
                }
                // Multi-line when annotations are present (readability beats the
                // single-line compactness once comments exist).
                if (properties.some((p) => p.includes("//"))) {
