...
if ok_native then
    ok("native module loads")
else
    err("native module", native)
end
...
if ok_native then
    ...
    if rtext(rn):match("alpha") then
        ok("namespaced name resolves")
    else
        err("namespaced", rtext(rn))
    end

    local unknown = native.dispatch("does_not_exist", {})
    if unknown.isError then
        ok("unknown tool returns isError")
    else
        err("unknown tool", "no error")
    end
end
...
if ok_native then
    ...
    local miss = rjson(
        native.dispatch("edit_buffer", { buffer = buf, diff = "<<<<<<< SEARCH\nNOPE\n=======\nX\n>>>>>>> REPLACE" })
    )
...
if ok_native then
    ...
    if not wf.isError then
        ok("write_file wrote a new file")
    else
        err("write_file", rtext(wf))
    end

    local rf = native.dispatch("read_file", { path = tmp .. ".out", start_line = 2, end_line = 2 })
    if rtext(rf) == "world" then
        ok("read_file honours line range")
    else
        err("read_file", rtext(rf))
    end

    local del = rjson(native.dispatch("delete_items", { paths = { tmp .. ".out" } }))
    if del and #del.deleted == 1 then
        ok("delete_items removed the file")
    else
        err("delete_items", vim.inspect(del))
    end
end
...
if ok_native then
    ...
    if res and res.cwd then
        ok("neovim://workspace resource")
    else
        err("workspace resource", vim.inspect(res))
    end
end
