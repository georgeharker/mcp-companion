...
package.loaded["mcp_companion.log"] = {
    ...
    info = function() end,
    warn = function(fmt, ...)
        table.insert(_captured_warns, string.format(fmt, ...))
    end,
    error = function() end,
}
...
test("find_root: finds project file in start dir", function()
    with_tempdir(function(tmp)
        ...
        assert_eq(
            file ~= nil and file:sub(-#".mcp-companion.json") == ".mcp-companion.json",
            true,
            "file path should end with .mcp-companion.json"
        )
    end)
end)
...
test("find_root: walks up to parent directory", function()
    with_tempdir(function(tmp)
        ...
        assert_eq(vim.fn.resolve(root), vim.fn.resolve(tmp), "should walk up to ancestor with the file")
    end)
end)

test("resolve: parses allowed_servers list", function()
    with_tempdir(function(tmp)
        write_file(tmp .. "/.mcp-companion.json", '{"allowed_servers": ["github", "gws"]}')
...
test("resolve: rejects mutually-exclusive fields, returns nil + warns", function()
    with_tempdir(function(tmp)
        write_file(tmp .. "/.mcp-companion.json", '{"allowed_servers": ["a"], "disabled_servers": ["b"]}')
        local cfg = project.resolve(tmp)
...
test("resolve: rejects non-string entries, returns nil + warns", function()
    with_tempdir(function(tmp)
        write_file(tmp .. "/.mcp-companion.json", '{"allowed_servers": ["github", 42]}')
        local cfg = project.resolve(tmp)
...
test("resolve_allowed: project allowed_servers wins over auto=true", function()
    with_tempdir(function(tmp)
        write_file(tmp .. "/.mcp-companion.json", '{"allowed_servers": ["github"]}')
...
test("resolve_allowed: drops unknown server names with a warning", function()
    with_tempdir(function(tmp)
        write_file(tmp .. "/.mcp-companion.json", '{"allowed_servers": ["github", "ghost"]}')
...
test("format_payload shortest: more disabled → allowed wins (shorter)", function()
    local p = project.format_payload({ "a", "b", "c" }, { "a", "b", "c", "d" }, "shortest")
...
test("format_payload shortest: more allowed → disabled wins (shorter)", function()
    local p = project.format_payload({ "a" }, { "a", "b", "c", "d" }, "shortest")
...
test("encode renders pretty JSON with $schema header", function()
    ...
    assert_eq(
        schema_idx ~= nil and field_idx ~= nil and schema_idx < field_idx,
        true,
...
test("save: discovers existing path when opts.path omitted", function()
    ...
    with_tempdir(function(tmp)
        ...
        local ok, err = pcall(function()
            local sub = tmp .. "/a/b"
            vim.fn.mkdir(sub, "p")
            write_file(tmp .. "/.mcp-companion.json", '{"allowed_servers": ["old"]}')
...
test("project_disabled_set: disabled_servers form is direct", function()
    local d = project.project_disabled_set({ disabled_servers = { "x" } }, { "a", "b", "x" })
    assert_eq(d.x, true)
    assert_eq(d.a, nil)
end)

test("project_disabled_set: allowed_servers inverted against known", function()
    local d = project.project_disabled_set({ allowed_servers = { "a" } }, { "a", "b", "c" })
    assert_eq(d.a, nil)
    assert_eq(d.b, true)
    assert_eq(d.c, true)
end)
...
test("toggle: no project file → creates one with the server hidden", function()
    with_tempdir(function(tmp)
        ...
        vim.cmd("cd " .. vim.fn.fnameescape(tmp))
        local ok, err = pcall(function()
            local result = project.toggle_in_project_file("clickup", { "github", "gws", "clickup" })
...
test("toggle: existing allowed_servers shape is preserved", function()
    with_tempdir(function(tmp)
        ...
        local ok, err = pcall(function()
            ...
            local result = project.toggle_in_project_file("gws", { "github", "gws", "clickup" })
...
test("toggle: existing disabled_servers shape is preserved", function()
    with_tempdir(function(tmp)
        ...
        local ok, err = pcall(function()
            ...
            local result = project.toggle_in_project_file("github", { "github", "gws", "clickup" })
            assert_eq(result.was_visible, true)
            assert_eq(result.now_visible, false)
            local cfg = project.resolve(tmp)
...
test("validate: tool_system_prompts must be boolean", function()
    with_tempdir(function(tmp)
        write_file(tmp .. "/.mcp-companion.json", '{"tool_system_prompts": "yes"}')
        local cfg = project.resolve(tmp)
...
test("resolve: parses tool_system_prompts=false", function()
    with_tempdir(function(tmp)
        write_file(tmp .. "/.mcp-companion.json", '{"tool_system_prompts": false}')
        local cfg = project.resolve(tmp)
        assert_eq(cfg ~= nil, true)
...
test("resolve: tool_system_prompts coexists with allowed_servers", function()
    with_tempdir(function(tmp)
        write_file(tmp .. "/.mcp-companion.json", '{"allowed_servers": ["github"], "tool_system_prompts": true}')
        local cfg = project.resolve(tmp)
        assert_eq(cfg ~= nil, true)
...
test("resolve_tool_system_prompts: project sets true overrides global=false", function()
    with_tempdir(function(tmp)
        write_file(tmp .. "/.mcp-companion.json", '{"tool_system_prompts": true}')
